...
from core.util.plugins import find_plugins
...
def get_services(services_folder):
    ...
    for i in possible_services:
        ...
        if isdir(location) and MAINMODULE + ".py" not in listdir(location):
            for j in listdir(location):
                name = join(location, j)
                if not isdir(name) or MAINMODULE + ".py" not in listdir(name):
                    continue
                try:
                    services.append(create_service_spec(name))
                except Exception:
                    LOG.error("Failed to create service from " + name, exc_info=True)
        if not isdir(location) or MAINMODULE + ".py" not in listdir(location):
            continue
        try:
...
class AudioService:
    ...
    def track_start(self, track):
        ...
        if track:
            ...
            self.bus.emit(Message("core.audio.playing_track", data={"track": track}))
        else:
    ...
    def _perform_stop(self):
        ...
        if self.current:
            ...
            if self.current.stop():
                self.bus.emit(Message("core.stop.handled", {"by": "audio:" + name}))
    ...
    def _track_info(self, message):
        ...
        if self.current:
            ...
            track_info = {}
        self.bus.emit(Message("core.audio.service.track_info_reply", data=track_info))
    ...
    def shutdown(self):
        ...
        self.bus.remove("core.audio.service.seek_forward", self._seek_forward)
        self.bus.remove("core.audio.service.seek_backward", self._seek_backward)
