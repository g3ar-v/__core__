...
from source.client.text.text_client import (
    connect_to_core,
    ctrl_c_handler,
    gui_main,
    load_settings,
...
from source.util import get_ipc_directory
...
def main():
    ...
    if "log_dir" in config:
        ...
        start_log_monitor(os.path.join(log_dir, "core.log"))
        start_log_monitor(os.path.join(log_dir, "listener.log"))
        start_log_monitor(os.path.join(log_dir, "audio.log"))
    else:
        start_log_monitor("/var/log/core/core.log")
        start_log_monitor("/var/log/core/listener.log")
