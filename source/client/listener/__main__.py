...
from source import CORE_ROOT_PATH, dialog
from source.client.listener.listener import ListenerLoop
from source.configuration import Configuration
from source.lock import Lock as PIDLock  # Create/Support PID locking file
...
def main(
    ...
    try:
        ...
        bus = start_message_bus_client("LISTENER")
        connect_bus_events(bus)
        callbacks = StatusCallbackMap(
            on_ready=ready_hook, on_error=error_hook, on_stopping=stopping_hook
        )
        status = ProcessStatus("listener", bus, callbacks)

        # Register handlers on internal ListenerLoop bus
        loop = ListenerLoop(watchdog)
