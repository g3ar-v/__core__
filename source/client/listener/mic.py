...
from source.util import (check_for_signal, get_ipc_directory, play_wav,
...
class ResponsiveRecognizer(speech_recognition.Recognizer):
    ...
    def __init__(self, wake_word_recognizer, watchdog=None):
        ...
        self.silence_detector = SilenceDetector(
            ...
            min_seconds=vad_config.get("min_seconds", 0.5),
    ...
    def _record_phrase(
        ...
        with stopwatch:
            ...
            while num_chunks < max_chunks:
                ...
                # HACK: helps check silence after wakeword.
                # TODO: this should only execute if wakeword was triggered.
                # NOTE: Can I get the SilenceDetector to do it?
                if not self._listen_triggered:
                    if result.type == SilenceResultType.SILENCE:
                        # LOG.info(f"time for silence: {(time.time() - stopwatch.timestamp)}")
                        if (
                            stopwatch.timestamp
                            and (time.time() - stopwatch.timestamp)
                            >= self.MIN_SILENCE_AT_END
                        ):
                            LOG.info("Sufficient silence detected, ending recording.")
                            break
    ...
    def mute_and_confirm_listening(self, source):
        ...
        #     )
        if audio_file:
            LOG.debug(f"audio file source: {audio_file}")
            source.mute()
    ...
    def listen(
        ...
        # if USER started initial conversation do not add wakeword audio
        if self._listen_triggered:
            LOG.info("listen triggered by system")
            audio_data = self._create_audio_data(frame_data, source)
            self._listen_triggered = False
        else:
            audio_data = self._create_audio_data(ww_data.audio + frame_data, source)
